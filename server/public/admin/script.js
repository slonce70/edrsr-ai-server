...
async function loadUsers(page = 1, search = '') {
    ...
    tbody.innerHTML = response.users.map(user => `
        <tr>
            ...
            <td>
                ${user.is_admin ? '<span class="status-badge status-completed">Admin</span>' : 
...
async function loadSecurityStats() {
    try {
        ...
        if (response.blockedIPs && response.blockedIPs.length > 0) {
            blockedList.innerHTML = response.blockedIPs.map(item => `
        ...
        if (response.failedAttempts && response.failedAttempts.length > 0) {
            attemptsList.innerHTML = response.failedAttempts.map(item => `
...
async function deleteUser(userId, email) {
    if (!confirm(`Удалить пользователя ${email}? Это действие нельзя отменить и удалит все данные пользователя.`)) return;
