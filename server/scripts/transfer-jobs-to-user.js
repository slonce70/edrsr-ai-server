...
async function transferJobsToUser(targetEmail) {
  try {
    ...
    const stats = {
      ...
      jobResults: await database.get('SELECT COUNT(*) as count FROM job_results WHERE user_id IS NULL'),
      chatMessages: await database.get('SELECT COUNT(*) as count FROM chat_messages WHERE user_id IS NULL'),
      parsedCases: await database.get('SELECT COUNT(*) as count FROM parsed_cases WHERE user_id IS NULL')
    };
    ...
    const totalItems = stats.jobs.count + stats.jobLinks.count + stats.jobResults.count + stats.chatMessages.count + stats.parsedCases.count;
    ...
    const finalStats = {
      jobs: await database.get('SELECT COUNT(*) as count FROM jobs WHERE user_id = $1', [targetUser.id]),
      jobLinks: await database.get('SELECT COUNT(*) as count FROM job_links WHERE user_id = $1', [targetUser.id]),
      jobResults: await database.get('SELECT COUNT(*) as count FROM job_results WHERE user_id = $1', [targetUser.id]),
      chatMessages: await database.get('SELECT COUNT(*) as count FROM chat_messages WHERE user_id = $1', [targetUser.id]),
      parsedCases: await database.get('SELECT COUNT(*) as count FROM parsed_cases WHERE user_id = $1', [targetUser.id])
    };
