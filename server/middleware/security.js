    ...
    if (res.statusCode === 401) {
      ...
      if (email) {
        ...
        if (emailAttempts.count >= 5) {
          ...
          logger.warn(`[SECURITY] Email ${email} blocked for 30 minutes after ${emailAttempts.count} failed attempts`);
        }
      }
      ...
      if (ipAttempts.count >= 10) {
        ...
        logger.warn(`[SECURITY] IP ${ip} blocked for 1 hour after ${ipAttempts.count} failed attempts`);
      }
      
      logger.warn(`[SECURITY] Failed admin login attempt from IP: ${ip}, Email: ${email || 'unknown'}`);
...
export function logSuspiciousActivity(req, res, next) {
  ...
  const unusualUserAgents = [
    /curl/i, /wget/i, /python/i, /go-http/i, /libwww/i
  ];
